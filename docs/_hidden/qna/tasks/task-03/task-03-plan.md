...
1. The watcher validates the file on write and either sends hidden repair feedback or marks the request ready.
1. `index.ts` locks the ready request and launches `showQuestionRuntimeFormShell(...)`.
1. `form-shell.ts` flattens the full authored tree once with `flattenQuestionsPreOrder(...)`.
1. `form-state.ts` creates one blank draft per flattened question and the shell renders all tabs at once.
1. Submit or cancel returns a shell-local result, but `index.ts` drops it, so no structured data reaches the agent or product layer.
...
1. For each parent -> child occurrence:
...
1. Merge repeated canonical nodes by `questionId` when signatures match.
1. Union outgoing edges across repeated matching occurrences.
1. Build active candidates by traversing from roots:
   - root depth is `0`
...
1. Merge active candidates by `questionId`:
   - keep minimum activation depth
   - append all current visibility reasons
1. Stable-toposort the active candidate set by `dependsOnQuestionIds`, using first-seen `questionOrder` as the tie-break.
1. Suppress any candidate whose dependency is absent, unresolved, or part of a dependency cycle.
1. Return the final `ActiveQuestionView` in current visible order.

#### Structured submission flow

1. Shell mutations update canonical drafts only.
1. Active view is recomputed from canonical graph plus current drafts.
1. Review summaries and blockers consume only the active view.
1. Submit builds an active-only structured submit envelope plus the latest canonical `draftSnapshot`.
1. Cancel builds only the latest canonical `draftSnapshot`.
1. `index.ts` sends one hidden structured message after the shell closes.
...
- `Key logic to add or change:`
  ...
  - Preserve deterministic issue order in this exact sequence:
    ...
    1. pre-order occurrence issues
    1. conflicting repeated-definition issues
    1. dependency reference issues
    1. activation reference issues
    1. `draftSnapshot` structural and reference issues
...
1. Trigger the same shared follow-up through two current parents and confirm:
   - it appears only once
   - numbering updates from the active view
   - `Visible because` explains both current paths
1. Answer a parent so a child appears, type into the child, then change the parent so the branch disappears.
   - confirm the child draft is hidden, not cleared
   - re-enable the branch and confirm the draft returns
1. Mark a dependency prerequisite `open`, `skipped`, and `needs_clarification` in turn and confirm the dependent stays hidden each time.
1. Submit a partial response and confirm:
...
1. Submit a form with no explicit active outcomes and confirm the hidden result is `kind: "no_user_response"`.
1. Cancel after edits and confirm the hidden cancel result carries the full `draftSnapshot` for later restoration.
1. Reopen the same form with that `draftSnapshot` and confirm compatible drafts restore, including hidden inactive branch drafts.
